...
import publish as P
FOLDERS = ["2026-08-05_life-begins-to-speak"]
token = os.environ["IG_TOKEN"].strip(); ig_id = os.environ["IG_USER_ID"].strip()
raw_base = os.environ["RAW_BASE"].rstrip("/")
today = datetime.now(ZoneInfo("Europe/Berlin")).strftime("%Y-%m-%d")
for folder in FOLDERS:
    with open(f"reels/{folder}/caption.txt", encoding="utf-8") as f: caption = f.read().strip()
    ...
    with open("posted.log", "a", encoding="utf-8") as f:
        f.write(f"{today} {folder} [REEL-EXTRA] {info.get('permalink','?')}\n")
